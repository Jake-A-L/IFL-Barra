...
class PeriodRisefactor(object):
    ...
    def __init__(self, stock_price_data, stock_price_calendar_tag,
        ...
        self.cur_original_period_rise_factor = {}
        self.cur_daily_rise_factor = {}
        ...
        self.second_last_close_price = {}

    def calculate_second_last_close_price(self, trade_time):
        self.second_last_close_price = {}
        if trade_time == 1:
        ...
        for line in cur_day_data:
            # self.second_last_close_price[line[0]] = line[12] old data
            self.second_last_close_price[line[0]] = line[11]  # new data

    def calculate_period_rise_factor(self, trade_time):
        self.cur_original_daily_rise_factor = {}
        self.cur_average_price = {}
        self.last_close_price = {}
        ...
        for line in cur_day_data:
            # self.last_close_price[line[0]] = line[12] old data
            self.last_close_price[line[0]] = line[11]  # new data

        for line in cur_day_data:
            if line[-1] == '交易' and line[4] != line[5]:
                # self.cur_average_price[line[0]] = line[10] * line[11] old data
                self.cur_average_price[line[0]] = line[17] * line[16]  # new data
                if line[0] in self.last_close_price and line[0] in self.second_last_close_price:
                    self.cur_original_period_rise_factor[line[0]] = (self.last_close_price[line[0]] -
                                                                    self.second_last_close_price[line[0]]) \
                                                                   / self.second_last_close_price[line[0]]
        self.cur_daily_rise_factor = sorted(self.cur_original_period_rise_factor.items(), key=lambda kv: kv[1])

    def divide_factor_data(self):  # divide factor data
        print(self.cur_daily_rise_factor)
        each_set_num = int(len(self.cur_daily_rise_factor) / self.set_num)
        tail_num = len(self.cur_daily_rise_factor) - self.set_num * each_set_num
        ...
        for i in range(self.set_num):
            ...
            i -= 1
            self.cur_divided_data.append(self.cur_daily_rise_factor[start:end])
            start = end
    ...
    def execute_factor(self):
        ...
        while trade_day <= len(self.stock_price_calendar_tag):
            print(trade_day)
            self.calculate_second_last_close_price(trade_day-2)
        ...
        pd.DataFrame(self.stock_yield_answer).to_csv('daily_rise_result.csv')
        self.draw_answer()
